...
public enum FieldEnum {
    ...
    /**
     * @return
     * @Description 不显示字段类型  0仪表盘不显示 默认字段  1仅数字类型 2仅地图类型字段
     * @Author UNIQUE
     * @Date 2022/11/24
     * @Param
     **/
    public static List<FieldEnum> getNoShowFieldEnum(Integer mapFlag) {
        List<FieldEnum> biDashboardNoShowFieldEnum = new ArrayList<>();
        switch (mapFlag) {
            case 0:
                biDashboardNoShowFieldEnum = Arrays.asList(FieldEnum.FILE, FieldEnum.DATE_INTERVAL, FieldEnum.HANDWRITING_SIGN, FieldEnum.DESC_TEXT, FieldEnum.DETAIL_TABLE, FieldEnum.CALCULATION_FUNCTION, FieldEnum.FIELD_GROUP, FieldEnum.RTF
                );
                break;
            case 1:
                biDashboardNoShowFieldEnum = Arrays.asList(FieldEnum.NUMBER, FieldEnum.FLOATNUMBER, FieldEnum.PERCENT
                );
                break;
            case 2:
                biDashboardNoShowFieldEnum = Arrays.asList(FieldEnum.MAP_ADDRESS, FieldEnum.ADDRESS, FieldEnum.AREA_POSITION, FieldEnum.CURRENT_POSITION
                );
                break;
            default:
        }
